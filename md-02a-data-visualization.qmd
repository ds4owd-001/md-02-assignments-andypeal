...
### Code chunk for Boxplot of continent vs life expectancy
...
### Code chunk for Timeseries plot of year vs life expectancy
...
### Code chunk for Barplot of continent vs count - using 'geom_col()
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: I expect that geobar is 'counting' the number of countries within each continent. Eg in Oceania there are 2 countries in the dataset.

### Code chunk for Barplot of continent vs count - using 'geom_bar()
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: first and second numbers set the lower and upper limit respectively for the x-axis, and third number sets the minor interval for the y-axis.

### Code chunk for Histogram
...
### Code chunk for Scatterplot and faceting
